...
use Awz\Bpsearch\Access\Custom\ComponentConfig;
                ...
                window.awz_right_dialog = {
                    ...
                    reloadItems: function(items, event){
                        ...
                        items.forEach(function(itm){
                            if(itm.getEntityId()==='user' || itm.getEntityType()==='awzbpsearch-user'){
                                let itmEv = {
                                    type: 'users',
                                    id: 'U'+itm.getId(),
                                    key: 'U'+itm.getId(),
                            ...
                            }else if(itm.getEntityType()==='awzbpsearch-group'){
                                let itmEv = {
                                    type: 'groups',
                                    id: 'G'+itm.getId(),
                                    key: 'G'+itm.getId(),
                    ...
                    load: function(groupKey, options){
                        ...
                        if(groupData.hasOwnProperty('members')){
                            for(k in groupData['members']){
                                ...
                                if(itm.type==='users'){
                                    preselectedItems.push(['awzbpsearch-user',itm.id]);
                                }
                                else if(itm.type==='awzbpsearch-user'){
                                    preselectedItems.push(['awzbpsearch-user',itm.id]);
                                }
                                else if(itm.type==='awzbpsearch-group'){
                                    preselectedItems.push(['awzbpsearch-group',itm.id]);
                                }
